...
class TextQueryRecommendation:
    ...
    def __init__(self, OPTIONS, datas, numOfNeighbours, min_libs, alpha, beta, gamma, p, q, if_C, if_apk):
    ...
    def start(self):
        ...
        for testingIcon in testingIcons:
            ...
            pic_dir = "simpic" + str(testingIcon)
            check_and_mk_dir(pic_dir)
            writeScores(pic_dir, testingIcon, dict2sortedlist(topnSim))

            apk_name = self.datas[testingIcon][5]
            path_name = self.datas[testingIcon][8]
            pic2 = get_md5(path_name)
            pic_true_name = "~/gator-3.8/ICON_CODE/" + apk_name + "/" + pic2 + ".png"
            CMD = "cp " + pic_true_name + " " + pic_dir + "/main.png"
            os.system(CMD)

            for item in topnSim:
                apk_name = self.datas[item][5]
                path_name = self.datas[item][8]
                if ";" in path_name:
                    for tmp in path_name.split(";"):
                        pic2 = get_md5(tmp)
                        pic_true_name = "~/gator-3.8/ICON_CODE/" + apk_name + "/" + pic2.strip() + ".png"
                        CMD = "cp " + pic_true_name + " " + pic_dir + "/" + str(item) + "_" + pic2 + ".png"
                        os.system(CMD)
                else:
                    pic2 = get_md5(path_name)
                    pic_true_name = "~/gator-3.8/ICON_CODE/" + apk_name + "/" + pic2.strip() + ".png"
